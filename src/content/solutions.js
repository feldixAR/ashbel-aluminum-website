export const trustItems = [
  "עבודה מול תוכניות",
  "ייצור וביצוע",
  "פתרונות לבית ולפרויקט",
  "שירותי מפעל לקבלנים ומתקינים"
]

export const solutionsIntro = {
  "title": "מוצרים ופתרונות",
  "subtitle": "תחומי העבודה המרכזיים של אשבל: פתחים, הצללות, פתרונות היקפיים ושירותי מפעל לפי תוכנית, מידה או מפרט."
}

export const solutions = [
  {
    "id": "modern",
    "slug": "סגנון-מודרני",
    "title": "סגנון מודרני",
    "displayTitle": "סגנון מודרני",
    "text": "חלונות, ויטרינות ומפתחים רחבים בקווים נקיים.",
    "image": "media/projects/hero-wide-openings.webp",
    "alt": "ויטרינות אלומיניום כהות במפתח רחב בבית פרטי",
    "includes": [
      "ויטרינות רחבות",
      "מערכות הזזה",
      "חלונות בקווים נקיים",
      "פתחים גדולים לפי תוכנית"
    ],
    "checks": [
      "רוחב וגובה הפתח",
      "מסילות וניקוז",
      "סוג זכוכית",
      "הכנות תריס או הצללה"
    ],
    "options": [
      {
        "title": "ויטרינות רחבות",
        "text": "פתחים גדולים לבית פרטי, סלון, יציאה למרפסת או חלל אירוח.",
        "image": "media/projects/hero-wide-openings.webp",
        "alt": "ויטרינות אלומיניום רחבות בחלל מגורים",
        "order": 1
      },
      {
        "title": "מערכות הזזה",
        "text": "פתרונות פתיחה בקווים נקיים לפי מידה, מסילות ותנאי השטח.",
        "image": "media/projects/sliding-vitrine-balcony.webp",
        "alt": "ויטרינת הזזה כהה ביציאה למרפסת",
        "order": 2
      },
      {
        "title": "חלונות בקווים נקיים",
        "text": "חלונות אלומיניום מינימליסטיים לחדרים, חזיתות וחללים מוארים.",
        "image": "media/projects/large-sliding-vitrine.webp",
        "alt": "חלונות אלומיניום רחבים עם קווים נקיים",
        "order": 3
      }
    ],
    "order": 1,
    "showOnHome": true,
    "legacySlugs": [],
    "gallery": [
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-interior-vitrine-02.jpg",
        "alt": "ויטרינת אלומיניום כהה בחלל בית פרטי בשיפוץ, תמונה 1",
        "title": "ויטרינה רחבה בבית בשיפוץ",
        "order": 1
      },
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-vitrine-shutters-03.jpg",
        "alt": "ויטרינת אלומיניום ותריסים חומים בחזית בית פרטי, תמונה 2",
        "title": "ויטרינה עם תריס חיצוני",
        "order": 2
      },
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-horizontal-window-06.jpg",
        "alt": "חלון אלומיניום אופקי כהה בחדר בבית פרטי בשיפוץ, תמונה 3",
        "title": "חלון אופקי לפי מידה",
        "order": 3
      }
    ]
  },
  {
    "id": "belgian",
    "slug": "סגנון-כפרי-בלגי",
    "title": "סגנון כפרי / בלגי",
    "displayTitle": "סגנון כפרי",
    "text": "חלונות ודלתות במראה קלאסי, עם חלוקות והתאמה לסגנון הבית.",
    "image": "media/projects/belgian-partition.webp",
    "alt": "מחיצת אלומיניום שחורה בסגנון בלגי",
    "includes": [
      "חלונות בלגיים",
      "דלתות אלומיניום",
      "חלוקות דקורטיביות",
      "מחיצות פנים"
    ],
    "checks": [
      "חלוקות ומידות",
      "כיוון פתיחה",
      "גוון וגימור",
      "שילוב עם פרטי נגרות או אבן"
    ],
    "options": [
      {
        "title": "חלונות בלגיים",
        "text": "חלוקות עדינות ומראה קלאסי שמתאים לבית כפרי או לחלל פנים מעוצב.",
        "image": "media/projects/belgian-partition.webp",
        "alt": "מחיצת אלומיניום שחורה בסגנון בלגי",
        "order": 1
      },
      {
        "title": "דלתות אלומיניום",
        "text": "דלתות פנים וחוץ לפי מידה, חלוקה, גוון וכיוון פתיחה.",
        "image": "media/projects/ribbed-glass-door.webp",
        "alt": "דלת אלומיניום שחורה עם זכוכית מחוספסת",
        "order": 2
      },
      {
        "title": "מחיצות פנים",
        "text": "מחיצות אלומיניום וזכוכית לחלוקה נקייה של חללים.",
        "image": "media/projects/belgian-partition.webp",
        "alt": "מחיצת פנים מאלומיניום וזכוכית",
        "order": 3
      }
    ],
    "order": 2,
    "showOnHome": true,
    "legacySlugs": [],
    "gallery": []
  },
  {
    "id": "shading",
    "slug": "תריסים-ופתרונות-הצללה",
    "title": "תריסים ופתרונות הצללה",
    "displayTitle": "הצללה",
    "text": "תריסים, רפפות ומערכות הצללה לשליטה באור ופרטיות.",
    "image": "media/projects/exterior-shutters.webp",
    "alt": "תריסים כהים בחזית אבן בהירה",
    "includes": [
      "תריסי גלילה",
      "רפפות",
      "הכנות למנועים",
      "פתרונות פרטיות והצללה"
    ],
    "checks": [
      "גודל הפתח",
      "מיקום ארגז",
      "חשמל וגישה לתחזוקה",
      "רמת אור ופרטיות נדרשת"
    ],
    "options": [
      {
        "title": "תריסי גלילה",
        "text": "תריסים לפתחים, חדרים וויטרינות לפי מידה והכנות חשמל.",
        "image": "media/projects/exterior-shutters.webp",
        "alt": "תריסים כהים בחזית אבן בהירה",
        "order": 1
      },
      {
        "title": "רפפות",
        "text": "שליטה באור, אוורור ופרטיות עם מראה נקי ומתוכנן.",
        "image": "media/projects/pergola-slats.webp",
        "alt": "רפפות אלומיניום ופרטי הצללה",
        "order": 2
      },
      {
        "title": "הכנות למנועים",
        "text": "תיאום ארגזים, חשמל וגישה לתחזוקה לפני ייצור והתקנה.",
        "image": "media/projects/exterior-shutters.webp",
        "alt": "חזית עם תריסים כהים ופתחים רחבים",
        "order": 3
      }
    ],
    "order": 3,
    "showOnHome": true,
    "legacySlugs": [],
    "gallery": [
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-shutter-front-04.jpg",
        "alt": "תריס אלומיניום חום עם שלבים אופקיים בחזית בית, תמונה 1",
        "title": "תריס אלומיניום בחזית",
        "order": 1
      },
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-interior-shutters-05.jpg",
        "alt": "חדר בשיפוץ עם חלונות ותריסי אלומיניום כהים, תמונה 2",
        "title": "חלונות ותריסים בחדר בשיפוץ",
        "order": 2
      },
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-louver-detail-07.jpg",
        "alt": "תקריב של שלבי רפפה מאלומיניום בגוון כהה, תמונה 3",
        "title": "פרט רפפה מאלומיניום",
        "order": 3
      },
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-louver-closeup-10.jpg",
        "alt": "חלון רפפה מאלומיניום כהה בתוך חדר בשיפוץ, תמונה 4",
        "title": "רפפה פנימית כהה",
        "order": 4
      }
    ]
  },
  {
    "id": "pergolas",
    "slug": "פרגולות-וגדרות-אלומיניום",
    "title": "פרגולות וגדרות אלומיניום",
    "displayTitle": "פרגולות",
    "text": "פרגולות, גדרות ואלמנטים חיצוניים לפי מידה.",
    "image": "media/projects/pergola-slats.webp",
    "alt": "פרגולת אלומיניום עם סרגלי הצללה",
    "includes": [
      "פרגולות אלומיניום",
      "גדרות ושערים",
      "אלמנטים חיצוניים",
      "פתרונות לפי מידה"
    ],
    "checks": [
      "כיוון שמש",
      "קיבועים וניקוז",
      "גובה ומפתח",
      "הכנות תאורה או חשמל"
    ],
    "options": [
      {
        "title": "פרגולות אלומיניום",
        "text": "הצללה נקייה לחצר, מרפסת או אזור ישיבה לפי מידה.",
        "image": "media/projects/pergola-slats.webp",
        "alt": "פרגולת אלומיניום עם סרגלי הצללה",
        "order": 1
      },
      {
        "title": "גדרות ושערים",
        "text": "אלמנטים חיצוניים מאלומיניום בתיאום עם חזית הבית.",
        "image": "media/projects/modern-facade-black-aluminum.webp",
        "alt": "חזית בית עם עבודות אלומיניום שחורות",
        "order": 2
      },
      {
        "title": "פתרונות חוץ לפי מידה",
        "text": "פרטים משלימים לחוץ הבית לפי מפרט, גמר וצבע.",
        "image": "media/projects/pergola-slats.webp",
        "alt": "פרטי אלומיניום חיצוניים להצללה",
        "order": 3
      }
    ],
    "order": 4,
    "showOnHome": true,
    "legacySlugs": [],
    "gallery": []
  },
  {
    "id": "envelope",
    "slug": "פתרונות-היקפיים-מאלומיניום",
    "title": "פתרונות היקפיים מאלומיניום",
    "displayTitle": "פתרונות היקפיים",
    "text": "חיפויי קירות, מסתורי כביסה, מסתורי מזגנים, מסתורים לדודי שמש/גז, סגירות חוץ, מעקות זכוכית, מקלחונים ופרטי אלומיניום משלימים.",
    "image": "media/projects/modern-facade-black-aluminum.webp",
    "alt": "חזית בית עם עבודות אלומיניום שחורות ומראה נקי",
    "includes": [
      "חיפויי קירות",
      "מסתורים ותשתיות חוץ",
      "סגירות ומעקות",
      "פרטי אלומיניום משלימים"
    ],
    "checks": [
      "גישה לתחזוקה",
      "איוורור",
      "חיבור לחזית",
      "עמידות חוץ וגמר צבע"
    ],
    "options": [
      {
        "title": "חיפויי קירות",
        "text": "פרטי אלומיניום לחזיתות ואזורים חיצוניים במראה נקי.",
        "image": "media/projects/modern-facade-black-aluminum.webp",
        "alt": "חזית בית עם אלומיניום שחור ומראה נקי",
        "order": 1
      },
      {
        "title": "מסתורים ותשתיות חוץ",
        "text": "מסתורי כביסה, מזגנים ודודי שמש או גז תוך שמירה על אוורור וגישה.",
        "image": "media/projects/modern-facade-black-aluminum.webp",
        "alt": "פרטי אלומיניום שחורים בחזית בית",
        "order": 2
      },
      {
        "title": "מעקות ומקלחונים",
        "text": "פתרונות משלימים מזכוכית ואלומיניום לפי מידה וגמר.",
        "image": "media/projects/narrow-window-stone.webp",
        "alt": "חלון אלומיניום צר בגוון כהה בקיר אבן",
        "order": 3
      }
    ],
    "order": 5,
    "showOnHome": true,
    "legacySlugs": [],
    "gallery": [
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-exterior-renovation-01.jpg",
        "alt": "חזית בית פרטי בשיפוץ עם ויטרינות ותריסי אלומיניום כהים, תמונה 1",
        "title": "חזית בית בשלב ביצוע",
        "order": 1
      },
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-vitrine-shutters-03.jpg",
        "alt": "ויטרינת אלומיניום ותריסים חומים בחזית בית פרטי, תמונה 2",
        "title": "ויטרינות ותריסים חיצוניים",
        "order": 2
      }
    ]
  },
  {
    "id": "factory",
    "slug": "שירותי-מפעל-לקבלנים-ומתקינים",
    "title": "שירותי מפעל לקבלנים ומתקינים",
    "displayTitle": "שירותי מפעל",
    "text": "חיתוך, עיבוד, צביעה, אספקת פרופילים וקיטים מוכנים להרכבה לפי מידה, מפרט או רשימת חיתוך.",
    "image": "media/projects/narrow-window-stone.webp",
    "alt": "חלון אלומיניום צר בגוון אנתרציט בקיר אבן",
    "includes": [
      "חיתוך לפי מידה",
      "עיבוד פרופילים",
      "צביעה",
      "קיטים מוכנים להרכבה"
    ],
    "checks": [
      "מפרט או רשימת חיתוך",
      "כמויות",
      "גוון וגימור",
      "לוחות זמנים ואופן אספקה"
    ],
    "options": [
      {
        "title": "חיתוך ועיבוד",
        "text": "חיתוך לפי מידה ועיבוד פרופילים בהתאם למפרט או רשימת חיתוך.",
        "image": "media/projects/narrow-window-stone.webp",
        "alt": "חלון אלומיניום צר בגוון אנתרציט בקיר אבן",
        "order": 1
      },
      {
        "title": "צביעה וגימור",
        "text": "התאמת גוון וגמר לפי דרישות הפרויקט והמערכות.",
        "image": "media/projects/ribbed-glass-door.webp",
        "alt": "דלת אלומיניום שחורה עם זכוכית מחוספסת",
        "order": 2
      },
      {
        "title": "קיטים מוכנים להרכבה",
        "text": "אספקה מסודרת לקבלנים ומתקינים לפי כמות, מידה ולוח זמנים.",
        "image": "media/projects/large-sliding-vitrine.webp",
        "alt": "מערכת אלומיניום רחבה מוכנה להתקנה",
        "order": 3
      }
    ],
    "order": 6,
    "showOnHome": true,
    "legacySlugs": [],
    "gallery": []
  }
]

export const processIntro = {
  "title": "מהתוכנית לשטח",
  "subtitle": "תהליך עבודה שמתחיל בבדיקה של החומר הקיים וממשיך לייצור, התקנה או אספקה לפי צורך."
}

export const processSteps = [
  {
    "title": "קבלת תוכניות / תמונות מהשטח",
    "text": "מקבלים תוכנית אלומיניום, תוכנית אדריכלית, מפרט, כתב כמויות או תמונות מהשטח.",
    "image": "media/projects/hero-wide-openings.webp",
    "alt": "מפתח ויטרינה רחב לבדיקת תוכנית אלומיניום",
    "order": 1,
    "images": [
      {
        "image": "media/projects/hero-wide-openings.webp",
        "alt": "מפתח ויטרינה רחב לבדיקת תוכנית אלומיניום",
        "order": 1,
        "primary": true
      }
    ]
  },
  {
    "title": "בדיקת פתחים ומפרט",
    "text": "בודקים מידות, כיווני פתיחה, זכוכית, תריסים, הכנות ותיאום מול התוכנית.",
    "image": "media/projects/large-sliding-vitrine.webp",
    "alt": "ויטרינה לבנה רחבה בחלל מגורים",
    "order": 2,
    "images": [
      {
        "image": "media/projects/large-sliding-vitrine.webp",
        "alt": "ויטרינה לבנה רחבה בחלל מגורים",
        "order": 1,
        "primary": true
      }
    ]
  },
  {
    "title": "מדידה ותיאום באתר",
    "text": "כאשר צריך, מתאמים מדידה בשטח ומוודאים שהפתחים וההכנות מתאימים לביצוע.",
    "image": "media/projects/herzliya-pituach/herzliya-pituach-interior-vitrine-02.jpg",
    "alt": "ויטרינת אלומיניום כהה בחלל בית פרטי בשיפוץ, תמונה 1",
    "order": 3,
    "images": [
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-interior-vitrine-02.jpg",
        "alt": "ויטרינת אלומיניום כהה בחלל בית פרטי בשיפוץ, תמונה 1",
        "order": 1,
        "primary": true
      }
    ]
  },
  {
    "title": "ייצור וגימור",
    "text": "חותכים, מעבדים, צובעים ומכינים את המערכות לפי המידות והמפרט שאושרו.",
    "image": "media/projects/ribbed-glass-door.webp",
    "alt": "דלת אלומיניום שחורה עם זכוכית מחוספסת",
    "order": 4,
    "images": [
      {
        "image": "media/projects/ribbed-glass-door.webp",
        "alt": "דלת אלומיניום שחורה עם זכוכית מחוספסת",
        "order": 1,
        "primary": true
      }
    ]
  },
  {
    "title": "התקנה ובדיקת גמר",
    "text": "מתקינים באתר או מספקים לפי מידה, ובודקים פתיחה, סגירה, איטום וגמרים.",
    "image": "media/projects/herzliya-pituach/herzliya-pituach-installation-room-09.jpg",
    "alt": "חלל בית פרטי בשיפוץ עם ויטרינות אלומיניום ועבודת התקנה, תמונה 2",
    "order": 5,
    "images": [
      {
        "image": "media/projects/herzliya-pituach/herzliya-pituach-installation-room-09.jpg",
        "alt": "חלל בית פרטי בשיפוץ עם ויטרינות אלומיניום ועבודת התקנה, תמונה 2",
        "order": 1,
        "primary": true
      }
    ]
  }
]
