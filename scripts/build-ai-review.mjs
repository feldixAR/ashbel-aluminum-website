...
const html = `<!doctype html><html><head><meta charset="utf-8"><style>body{margin:0;background:#e8eef3;color:#121820;font-family:Arial,sans-serif}main{padding:18px}h1{font-size:26px;margin:0 0 6px}p{font-size:14px;margin:0 0 16px;color:#52606f}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}article{background:white;border:1px solid #ccd6df;overflow:hidden}h2{margin:0;padding:7px 9px;background:#17212b;color:white;font-size:12px;direction:ltr;text-align:left}img{display:block;width:100%;height:240px;object-fit:contain;object-position:top center;background:#f4f6f8}</style></head><body><main><h1>Ashbel AI Visual Review</h1><p>Compressed all-page screenshot sheet for AI inspection.</p><section class="grid">${cards}</section></main></body></html>`;

const browser = await chromium.launch();
const page = await browser.newPage({ viewport: { width: 1200, height: 1500 }, deviceScaleFactor: 1 });
await page.setContent(html, { waitUntil: 'load' });
await page.screenshot({ path: path.join(dir, 'ai-review-contact-sheet.jpg'), fullPage: true, type: 'jpeg', quality: 58 });
await browser.close();

const jpg = await readFile(path.join(dir, 'ai-review-contact-sheet.jpg'), 'base64');
await writeFile(path.join(dir, 'ai-review-embedded.html'), `<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ashbel AI Visual Review</title><style>body{margin:0;background:#eef3f6;font-family:Arial,sans-serif;color:#111827}main{max-width:1200px;margin:auto;padding:20px}img{width:100%;height:auto;border:1px solid #d6dee6;background:white}</style></head><body><main><h1>Ashbel AI Visual Review</h1><img src="data:image/jpeg;base64,${jpg}" alt="Ashbel AI visual review"></main></body></html>\n`, 'utf8');

console.log('AI visual review artifacts created.');
